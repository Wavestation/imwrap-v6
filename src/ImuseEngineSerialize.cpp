...
extern bool LoadImuseSequence(const SoundVariantView &variant, ImuseSequence *seq, std::string *error);
...
bool ImuseEngine::Serialize(std::ostream &os) const {
...
bool ImuseEngine::Deserialize(std::istream &is) {
    stopAllSounds();
    _activeSounds.clear();
    _triggerQueue.clear();
    ...
    for (uint32_t s = 0; s < activeSoundCount; ++s) {
        ...
        if (_bank && _bank->hasSound(snd.soundId)) {
            ...
            if (variant.valid()) {
                snd.variant = variant;
                LoadImuseSequence(snd.variant, &snd.sequence, nullptr);
