...
bool DebugMidiEnabled() {
    static const bool enabled = std::getenv("IMWRAP_DEBUG_MIDI") != nullptr;
    return enabled;
}
...
bool IMWrapEngine::MidiChannelState::hasActiveNotes() const {
...
int IMWrapEngine::HookState::queryParam(int param, uint8_t chan) const {
    switch (param) {
    case 18:
        return jump[0];
    case 19:
...
int IMWrapEngine::HookState::set(uint8_t cls, uint8_t value, uint8_t chan) {
    switch (cls) {
    case 0:
        if (value != jump[0]) {
            jump[1] = jump[0];
...
IMWrapEngine::ActiveSound *IMWrapEngine::findActiveSound(uint16_t soundId) {
    auto it = _activeSounds.find(soundId);
    return it != _activeSounds.end() ? &it->second : nullptr;
}

const IMWrapEngine::ActiveSound *IMWrapEngine::findActiveSound(uint16_t soundId) const {
    auto it = _activeSounds.find(soundId);
    return it != _activeSounds.end() ? &it->second : nullptr;
}

uint8_t IMWrapEngine::getChannelVolume(uint16_t volChan) const {
...
void IMWrapEngine::refreshEffectiveVolume(ActiveSound *sound) {
    if (!sound) {
        return;
    }
...
int IMWrapEngine::queryPartParam(const ActiveSound &sound, int param, uint8_t chan) const {
...
int IMWrapEngine::getSoundParam(const ActiveSound &sound, int param, uint8_t chan) const {
    switch (param) {
    case 0:
...
int IMWrapEngine::setSoundTranspose(ActiveSound *sound, uint8_t relative, int value) {
...
uint32_t IMWrapEngine::ticksPerBeat(const ActiveSound &sound) const {
    return sound.sequence.smf.ppqn() ? sound.sequence.smf.ppqn() : 480u;
}

uint32_t IMWrapEngine::absoluteTick(const ActiveSound &sound) const {
    return (static_cast<uint32_t>(sound.beatIndex > 0 ? sound.beatIndex - 1 : 0) * ticksPerBeat(sound)) + sound.tickIndex;
}

uint32_t IMWrapEngine::trackEndTick(const ActiveSound &sound, uint16_t track) const {
...
uint16_t IMWrapEngine::normalizeTrackIndex(const ActiveSound &sound, uint16_t requestedTrack) const {
...
void IMWrapEngine::setAbsoluteTick(ActiveSound *sound, uint32_t absoluteTickValue) {
    if (!sound) {
        return;
    }
...
bool IMWrapEngine::jumpTo(ActiveSound *sound, uint16_t track, uint16_t beat, uint16_t tick) {
    if (!sound || beat == 0) {
        return false;
    }
...
int IMWrapEngine::scanTo(ActiveSound *sound, uint16_t track, uint16_t beat, uint16_t tick) {
    if (!sound) {
        return -1;
    }
...
bool IMWrapEngine::setLoop(ActiveSound *sound, uint16_t count, uint16_t toBeat, uint16_t toTick, uint16_t fromBeat, uint16_t fromTick) {
...
bool IMWrapEngine::findNextControlTick(const ActiveSound &sound, uint16_t track, uint32_t afterTick, uint32_t *tick) const {
    bool found = false;
    uint32_t best = 0;

    for (const IMWrapControlEvent &event : sound.sequence.controlEvents) {
...
std::vector<IMWrapControlEvent> IMWrapEngine::collectControlEvents(const ActiveSound &sound, uint16_t track, uint32_t tick) const {
    std::vector<IMWrapControlEvent> events;
    for (const IMWrapControlEvent &event : sound.sequence.controlEvents) {
...
bool IMWrapEngine::findNextTrackEventTick(const ActiveSound &sound, uint16_t track, uint32_t afterTick, uint32_t *tick) const {
    if (track >= sound.sequence.smf.tracks.size()) {
        return false;
    }
...
void IMWrapEngine::resetMidiState(ActiveSound *sound) {
    if (!sound) {
        return;
    }
...
void IMWrapEngine::clearHangingNotes(ActiveSound *sound) {
    if (!sound) {
        return;
    }
...
void IMWrapEngine::cancelHangingNote(ActiveSound *sound, uint8_t channel, uint8_t note) {
...
void IMWrapEngine::emitMidiMessage(uint16_t soundId, uint8_t status, uint8_t data1, bool hasData2, uint8_t data2) {
    if (_midiSink) {
...
IMWrapEngine::PartState *IMWrapEngine::getActivePart(ActiveSound *sound, uint8_t channel) {
...
IMWrapEngine::PartState *IMWrapEngine::getPart(ActiveSound *sound, uint8_t channel) {
    if (!sound || channel >= sound->parts.size()) {
        return nullptr;
    }
...
void IMWrapEngine::initializePart(ActiveSound *sound, PartState *part, uint8_t channel) {
    if (!sound || !part) {
        return;
    }
...
void IMWrapEngine::clearPartActiveNotes(PartState *part) {
    if (!part) {
        return;
    }
...
uint8_t IMWrapEngine::computeEffectivePartVolume(const ActiveSound &sound, const PartState &part) const {
...
int8_t IMWrapEngine::computeEffectivePartPan(const ActiveSound &sound, const PartState &part) const {
    return static_cast<int8_t>(ClampInt(part.pan + sound.pan, -64, 63));
}

int8_t IMWrapEngine::computeEffectivePartTranspose(const ActiveSound &sound, const PartState &part) const {
    return static_cast<int8_t>(ClampInt(TransposeClamp(part.transpose + sound.transpose, -24, 24), -24, 24));
}

uint8_t IMWrapEngine::clampOutputNote(int note) const {
    return static_cast<uint8_t>(ClampInt(note, 0, 127));
}

bool IMWrapEngine::clearToTransmit(ActiveSound *sound, PartState *part) {
...
void IMWrapEngine::removeSuspendedPart(PartState *part) {
    if (!part) {
        return;
    }
...
void IMWrapEngine::suspendPart(PartState *part) {
    if (!part) {
        return;
    }
...
int IMWrapEngine::allocateOutputChannel(uint8_t priority) {
    ...
    if (_logCallback) {
        _logCallback("iMWrap: Channel dispute! Part (sound=" + std::to_string(best->ownerSoundId) + 
...
bool IMWrapEngine::reassignChannelAndResumePart(uint8_t outputChannel) {
...
void IMWrapEngine::reallocateMidiChannels() {
    while (true) {
        PartState *best = nullptr;
        ...
        if (outputChannel < 0) {
            if (_logCallback) {
                _logCallback("iMWrap Warning: No physical channel available for Part (sound=" + std::to_string(bestSound->soundId) + 
                             ", channel=" + std::to_string(best->channel) + 
                             ", priority=" + std::to_string(best->priorityEffective) + ")");
            }
            return;
        ...
        if (_logCallback) {
            _logCallback("iMWrap: Allocated physical channel " + std::to_string(outputChannel) + 
...
void IMWrapEngine::releaseOutputChannel(PartState *part) {
...
uint8_t IMWrapEngine::outputChannelForPart(const PartState &part) const {
    return part.percussion ? 9 : static_cast<uint8_t>(part.outputChannel);
}

void IMWrapEngine::emitPartMidiMessage(ActiveSound *sound, PartState *part, uint8_t statusClass, uint8_t data1, bool hasData2, uint8_t data2) {
    if (!sound || !part) {
        return;
    }
...
void IMWrapEngine::sendPartTranspose(ActiveSound *sound, PartState *part) {
...
void IMWrapEngine::applyPartPitchBendFactor(ActiveSound *sound, PartState *part) {
    if (!sound || !part) {
        return;
    }
...
void IMWrapEngine::applyPartPitchBend(ActiveSound *sound, PartState *part) {
    if (!sound || !part) {
        return;
    }
...
void IMWrapEngine::applyPartVolume(ActiveSound *sound, PartState *part) {
    if (!sound || !part) {
        return;
    }
...
void IMWrapEngine::applyPartPan(ActiveSound *sound, PartState *part) {
    if (!sound || !part) {
        return;
    }
...
void IMWrapEngine::applyPartModwheel(ActiveSound *sound, PartState *part) {
    if (!sound || !part) {
        return;
    }
...
void IMWrapEngine::applyPartSustain(ActiveSound *sound, PartState *part) {
    if (!sound || !part) {
        return;
    }
...
void IMWrapEngine::applyPartEffectLevel(ActiveSound *sound, PartState *part) {
    if (!sound || !part) {
        return;
    }
...
void IMWrapEngine::applyPartChorus(ActiveSound *sound, PartState *part) {
    if (!sound || !part) {
        return;
    }
...
void IMWrapEngine::applyPartPolyphony(ActiveSound *sound, PartState *part) {
    if (!sound || !part) {
        return;
    }
...
void IMWrapEngine::applyPartProgram(ActiveSound *sound, PartState *part) {
    if (!sound || !part) {
        return;
    }
...
void IMWrapEngine::sendMt32Sysex(uint16_t soundId, uint32_t addr, const uint8_t* data, size_t size) {
    if (!_midiSink) return;
...
void IMWrapEngine::sendPartCustomSysex(uint16_t soundId, PartState* part) {
...
void IMWrapEngine::applyPartAllState(ActiveSound *sound, PartState *part) {
    if (!sound || !part) {
        return;
    }
...
void IMWrapEngine::partAllNotesOff(ActiveSound *sound, PartState *part) {
    if (!sound || !part) {
        return;
    }
...
void IMWrapEngine::partOff(ActiveSound *sound, PartState *part) {
    if (!sound || !part) {
        return;
    }
...
void IMWrapEngine::partSetOnOff(ActiveSound *sound, uint8_t channel, bool on) {
...
void IMWrapEngine::partSetVolume(ActiveSound *sound, uint8_t channel, uint8_t volume) {
    PartState *part = getPart(sound, channel);
    if (!part) {
        return;
    }
...
void IMWrapEngine::partSetPan(ActiveSound *sound, uint8_t channel, int8_t pan) {
    PartState *part = getPart(sound, channel);
    if (!part) {
        return;
    }
...
void IMWrapEngine::partSetPriority(ActiveSound *sound, uint8_t channel, int8_t priority) {
    PartState *part = getPart(sound, channel);
    if (!part) {
        return;
    }
...
void IMWrapEngine::partSetTranspose(ActiveSound *sound, uint8_t channel, int8_t transpose) {
    PartState *part = getPart(sound, channel);
    if (!part) {
        return;
    }
...
void IMWrapEngine::partSetProgram(ActiveSound *sound, uint8_t channel, uint8_t program) {
    PartState *part = getPart(sound, channel);
    if (!part) {
        return;
    }
...
void IMWrapEngine::partSetPitchBend(ActiveSound *sound, uint8_t channel, int16_t value) {
    PartState *part = getPart(sound, channel);
    if (!part) {
        return;
    }
...
void IMWrapEngine::partSetPitchBendFactor(ActiveSound *sound, uint8_t channel, uint8_t value) {
    PartState *part = getPart(sound, channel);
    if (!part) {
        return;
    }
...
void IMWrapEngine::partSetPolyphony(ActiveSound *sound, uint8_t channel, uint8_t value) {
    PartState *part = getPart(sound, channel);
    if (!part) {
        return;
    }
...
void IMWrapEngine::partSetDetune(ActiveSound *sound, uint8_t channel, int16_t value) {
    PartState *part = getPart(sound, channel);
    if (!part) {
        return;
    }
...
void IMWrapEngine::partSetModwheel(ActiveSound *sound, uint8_t channel, uint8_t value) {
    PartState *part = getPart(sound, channel);
    if (!part) {
        return;
    }
...
void IMWrapEngine::partSetSustain(ActiveSound *sound, uint8_t channel, bool on) {
    PartState *part = getPart(sound, channel);
    if (!part) {
        return;
    }
...
void IMWrapEngine::partSetEffectLevel(ActiveSound *sound, uint8_t channel, uint8_t value) {
    PartState *part = getPart(sound, channel);
    if (!part) {
        return;
    }
...
void IMWrapEngine::partSetChorus(ActiveSound *sound, uint8_t channel, uint8_t value) {
    PartState *part = getPart(sound, channel);
    if (!part) {
        return;
    }
...
void IMWrapEngine::partSetInstrument(ActiveSound *sound, uint8_t channel, uint16_t value) {
    PartState *part = getPart(sound, channel);
    if (!part) {
        return;
    }
...
void IMWrapEngine::partNoteOn(ActiveSound *sound, uint8_t channel, uint8_t sourceNote, uint8_t velocity) {
    PartState *part = getPart(sound, channel);
    if (!sound || !part || !part->on) {
        return;
    }
...
void IMWrapEngine::partNoteOff(ActiveSound *sound, uint8_t channel, uint8_t sourceNote) {
    PartState *part = getPart(sound, channel);
    if (!sound || !part || !part->on) {
        return;
    }
...
void IMWrapEngine::turnOffParts(ActiveSound *sound) {
    if (!sound) {
        return;
    }
...
void IMWrapEngine::replayScanActiveNotes(ActiveSound *sound) {
    if (!sound) {
        return;
    }
...
bool IMWrapEngine::dispatchChannelEvent(ActiveSound *sound, uint8_t status, uint8_t data1, bool hasData2, uint8_t data2) {
    if (!sound) {
        return false;
    }
...
void IMWrapEngine::trackMidiMessage(ActiveSound *sound, uint8_t status, uint8_t data1, bool hasData2, uint8_t data2) {
    if (!sound) {
        return;
    }
...
void IMWrapEngine::flushActiveMidiState(uint16_t soundId) {
...
void IMWrapEngine::releaseSustainPedals(ActiveSound *sound) {
    if (!sound) {
        return;
    }
...
void IMWrapEngine::scheduleSmartJumpNotes(ActiveSound *sound) {
...
bool IMWrapEngine::findNextHangingTick(const ActiveSound &sound, uint32_t *tickDelta) const {
    uint32_t best = 0;
    bool found = false;
...
void IMWrapEngine::advanceHangingNotes(ActiveSound *sound, uint32_t deltaTicks) {
...
void IMWrapEngine::emitExpiredHangingNotes(ActiveSound *sound) {
    if (!sound || sound->hangingNotes.empty()) {
        return;
    }
...
bool IMWrapEngine::isScanActiveFor(uint16_t soundId) const {
    return _scan.active && _scan.soundId == soundId;
}

void IMWrapEngine::beginScan(uint16_t soundId) {
    _scan.reset();
    _scan.active = true;
    _scan.soundId = soundId;
}

void IMWrapEngine::endScan(ActiveSound *sound) {
...
void IMWrapEngine::trackScanNote(uint8_t status, uint8_t data1, bool hasData2, uint8_t data2) {
    const uint8_t channel = status & 0x0F;
    const uint8_t statusClass = status & 0xF0;
...
bool IMWrapEngine::handleMidiEvent(uint16_t soundId, const MidiEvent &event) {
    ActiveSound *sound = findActiveSound(soundId);
    if (!sound) {
        return false;
    }
...
bool IMWrapEngine::dispatchTrackTick(uint16_t soundId) {
    ActiveSound *sound = findActiveSound(soundId);
    if (!sound || sound->trackIndex >= sound->sequence.smf.tracks.size()) {
        return false;
    }
...
bool IMWrapEngine::maybeJump(ActiveSound *sound, uint8_t hookCommand, uint16_t track, uint16_t beat, uint16_t tick) {
    if (!sound) {
        return false;
    }
...
bool IMWrapEngine::executeControlEvent(uint16_t soundId, const IMWrapControlEvent &event) {
    ActiveSound *sound = findActiveSound(soundId);
    if (!sound) {
        return false;
    }
...
int IMWrapEngine::enqueueTrigger(uint16_t soundId, uint8_t marker) {
...
int IMWrapEngine::clearQueue() {
    _triggerQueue.clear();
    return 0;
}

int IMWrapEngine::queryQueue(uint16_t param) const {
    switch (param) {
    case 0:
...
int IMWrapEngine::setScriptTrigger(uint16_t soundId, uint8_t marker, const CommandPacket &command) {
...
int IMWrapEngine::clearScriptTrigger(int soundId, int marker) {
...
int IMWrapEngine::checkScriptTrigger(uint16_t soundId, int marker) const {
    int count = 0;
...
int IMWrapEngine::fireAllScriptTriggers(uint16_t soundId) {
...
void IMWrapEngine::addDeferredCommand(uint32_t ticksLeft, const CommandPacket &command) {
    if (command.argc == 0) {
        return;
    }
...
void IMWrapEngine::processDeferredCommands(uint32_t deltaTicks) {
...
bool IMWrapEngine::startSound(uint16_t soundId) {
    if (_logCallback) {
        _logCallback("IMWrapEngine::startSound(" + std::to_string(soundId) + ")");
    }
...
void IMWrapEngine::stopSound(uint16_t soundId) {
    if (_logCallback) {
        _logCallback("IMWrapEngine::stopSound(" + std::to_string(soundId) + ")");
    }
...
void IMWrapEngine::stopAllSounds() {
...
bool IMWrapEngine::isSoundActive(uint16_t soundId) const {
    return _activeSounds.find(soundId) != _activeSounds.end();
}

int IMWrapEngine::getSoundStatus(uint16_t soundId) const {
    if (isSoundActive(soundId)) {
        return 1;
    }
...
bool IMWrapEngine::getPlaybackLocation(uint16_t soundId, uint16_t *track, uint16_t *beat, uint16_t *tick) const {
...
uint32_t IMWrapEngine::currentTempoMicrosPerQuarter() const {
...
double IMWrapEngine::transportTicksPerSecond() const {
    if (_activeSounds.empty()) {
        return 960.0;
    }
...
void IMWrapEngine::advanceAll(uint32_t deltaTicks) {
...
int32_t IMWrapEngine::doCommand(const CommandPacket &packet) {
    return doCommand(packet.argc, packet.args.data());
}

int IMWrapEngine::dispatchGlobalCommand(uint8_t cmd, uint16_t argc, const int16_t *args) {
    switch (cmd) {
    case 6:
...
int IMWrapEngine::dispatchPlayerCommand(uint8_t cmd, uint16_t argc, const int16_t *args) {
    if (argc < 2) {
        return -1;
    }
...
int32_t IMWrapEngine::doCommand(uint16_t argc, const int16_t *args) {
    if (!args || argc < 1) {
        return -1;
    }
