...
struct ImuseSysExView: View {
    ...
    private var generatorView: some View {
        HSplitView {
            ...
            VStack {
                Form {
                    Section(header: Text("Type de Message iMUSE")) {
                ...
                GroupBox("Structure du Message") {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        HStack {
                            ...
                            Text("ID Manufacturier (iMUSE/SCUMM)")
                        }
                        HStack {
                        ...
                        HStack {
                            ...
                            VStack(alignment: .leading) {
                                Text(msg.typeName)
                                    .font(.title2)
                                    .fontWeight(.bold)
                                Text(String(format: "Code de commande iMUSE: 0x%02X", msg.code))
                                    .font(.caption)
                                    .foregroundColor(.secondary)
                            }
                        }
    ...
    private func parseHex() {
        ...
        if body[0] != 0x7D {
            parseError = "ID manufacturier iMUSE absent (devrait être 7D)."
            decodedMessage = nil
            return
        }
