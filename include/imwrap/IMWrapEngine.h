...
#ifndef IMWRAP_IMWRAP_ENGINE_H
#define IMWRAP_IMWRAP_ENGINE_H
...
#include "imwrap/IMWrapCommand.h"
...
private:
    ...
    std::vector<IMWrapControlEvent> collectControlEvents(const ActiveSound &sound, uint16_t track, uint32_t tick) const;
    ...
    bool executeControlEvent(uint16_t soundId, const IMWrapControlEvent &event);
    ...
    void processIMWrapGlobalCommand(int cmd, const std::vector<uint8_t>& payload);
