...
extension IMWrapAuthoringProfile: Hashable {}
...
public final class AuthoringViewModel: ObservableObject {
    ...
    @Published public var selectedProfile: IMWrapAuthoringProfile = IMWrapAuthoringProfileGeneralMidi {
        didSet {
            updateEngineProfile()
            reloadSoundList()
        }
    ...
    deinit {
        ...
        audioSourceNode = nil
        if let engineHandle {
            imwrap_engine_destroy(engineHandle)
        }
        if let bankHandle {
            imwrap_bank_destroy(bankHandle)
        }
        if midiOutputPort != 0 {
    ...
    public func loadBank() {
        ...
        let bank = imwrap_bank_create()
        var errorBuffer = [CChar](repeating: 0, count: 512)
        let ok = bankPath.withCString { pathPtr in
            imwrap_bank_load(bank, pathPtr, &errorBuffer, errorBuffer.count)
        }

        guard ok == 1 else {
            errorMessage = String(cString: errorBuffer)
            imwrap_bank_destroy(bank)
            return
        }

        let engine = imwrap_engine_create(bank)
        imwrap_engine_set_profile(engine, selectedProfile)
        let nativeMt32 = (outputBackend == .mt32emu || (outputBackend == .coreMidi && selectedProfile == IMWrapAuthoringProfileMt32)) ? 1 : 0
        imwrap_engine_set_native_mt32_output(engine, Int32(nativeMt32))
        
        // Setup Swift callbacks for CoreMIDI
        imwrap_engine_set_callbacks(engine, { (userData, soundId, status, data1, data2) in
    ...
    public func enablePreview() {
        guard let engineHandle else {
            errorMessage = "Charge d'abord une banque iMWrap."
            return
        }
    ...
    public func disablePreview() {
        ...
        imwrap_engine_disable_fluidsynth(engineHandle)
        imwrap_engine_disable_mt32(engineHandle)
        imwrap_engine_disable_adlib(engineHandle)
        previewEnabled = false
    }

    public func reloadSoundList() {
        guard let bankHandle else { return }
        let count = imwrap_bank_sound_count(bankHandle)
        var newSounds: [SoundSummary] = []
        newSounds.reserveCapacity(count)

        for index in 0..<count {
            let soundID = imwrap_bank_sound_id_at(bankHandle, index)
            var nameBuffer = [CChar](repeating: 0, count: 256)
            _ = imwrap_bank_sound_name(bankHandle, soundID, &nameBuffer, nameBuffer.count)
            let name = String(cString: nameBuffer)
            
            // Load tracks
            let trackCount = imwrap_bank_track_count(bankHandle, soundID, selectedProfile)
            ...
            for tIndex in 0..<trackCount {
                ...
                let ok = imwrap_bank_track_summary(bankHandle, soundID, selectedProfile, tIndex, &trackNameBuffer, trackNameBuffer.count, &eventCount)
                if ok == 1 {
    ...
    private func updateEngineProfile() {
        if let engineHandle {
            imwrap_engine_set_profile(engineHandle, selectedProfile)
            let nativeMt32 = (outputBackend == .mt32emu || (outputBackend == .coreMidi && selectedProfile == IMWrapAuthoringProfileMt32)) ? 1 : 0
            imwrap_engine_set_native_mt32_output(engineHandle, Int32(nativeMt32))
        }
    }
    ...
    public func reloadSelectedEvents() {
        ...
        let count = imwrap_bank_event_count(bankHandle, soundID, selectedProfile)
        ...
        for index in 0..<count {
            ...
            let ok = imwrap_bank_event_summary(bankHandle, soundID, selectedProfile, index, &tick, &track, &textBuffer, textBuffer.count)
            if ok == 1 {
    ...
    public func startSelectedSound() {
        ...
        if imwrap_engine_start_sound(engineHandle, soundID) == 0 {
    ...
    public func stopSelectedSound() {
        ...
        imwrap_engine_stop_sound(engineHandle, soundID)
        refreshActiveSounds()
    ...
    public func stopAllSounds() {
        ...
        imwrap_engine_stop_all(engineHandle)
        refreshActiveSounds()
        stopTransport()
    }
    ...
    public func advance(ticks: UInt32) {
        ...
        imwrap_engine_advance(engineHandle, ticks)
        refreshActiveSounds()
        if activeSounds.isEmpty {
            stopTransport()
        }
    ...
    public func applyHook() {
        ...
        if imwrap_engine_set_hook(engineHandle, soundID, hookClass, hookValue, hookChannel) == 0 {
    ...
    public func refreshActiveSounds() {
        ...
        let count = imwrap_engine_active_sound_count(engineHandle)
        var newActive: [ActiveSoundStatus] = []
        newActive.reserveCapacity(count)

        for index in 0..<count {
            let soundID = imwrap_engine_active_sound_id_at(engineHandle, index)
            var track: UInt16 = 0
            var beat: UInt16 = 0
            var tick: UInt16 = 0
            if imwrap_engine_get_location(engineHandle, soundID, &track, &beat, &tick) == 1 {
        ...
        let sourceNode = AVAudioSourceNode { _, _, frameCount, audioBufferList -> OSStatus in
            ...
            if buffers.count >= 2,
               ...
               let rightData = buffers[1].mData {
                ...
                switch backend {
                case .mt32emu:
                    imwrap_engine_render_mt32(previewEngineHandle, frameCount, left, right)
                case .adlib:
                    imwrap_engine_render_adlib(previewEngineHandle, frameCount, left, right)
                default:
                    imwrap_engine_render_fluidsynth(previewEngineHandle, frameCount, left, right)
                }
                ...
                switch backend {
                case .mt32emu:
                    imwrap_engine_render_mt32(previewEngineHandle, frameCount, mono, mono)
                case .adlib:
                    imwrap_engine_render_adlib(previewEngineHandle, frameCount, mono, mono)
                default:
                    imwrap_engine_render_fluidsynth(previewEngineHandle, frameCount, mono, mono)
                }
            }
            return 0
        }
    ...
    private func transportStep(elapsed: Double) {
        ...
        let ticksPerSecond = imwrap_engine_ticks_per_second(engineHandle)
        ...
        if wholeTicks > 0 {
            imwrap_engine_advance(engineHandle, wholeTicks)
            refreshActiveSounds()
        }
    ...
    private func setupCoreMIDI() {
        ...
        let status = MIDIClientCreateWithBlock("iMWrap MIDI Client" as CFString, &client, nil)
        if status == noErr {
            midiClient = client
            var port = MIDIPortRef()
            let portStatus = MIDIOutputPortCreate(midiClient, "iMWrap Output Port" as CFString, &port)
            if portStatus == noErr {
                midiOutputPort = port
            }
        }
    ...
    public func handleBackendChange() {
        ...
        case .fluidSynth:
            if !soundFontPath.isEmpty {
                ...
                let ok = soundFontPath.withCString { pathPtr in
                    imwrap_engine_enable_fluidsynth(engineHandle, pathPtr, &errorBuffer, errorBuffer.count)
                }
                if ok == 1 {
                    errorMessage = ""
                    startAudioOutput()
        ...
        case .adlib:
            var errorBuffer = [CChar](repeating: 0, count: 512)
            let ok = imwrap_engine_enable_adlib(engineHandle, &errorBuffer, errorBuffer.count)
            if ok == 1 {
                errorMessage = ""
                startAudioOutput()
            } else {
        ...
        case .mt32emu:
            if !mt32RomDirPath.isEmpty {
                ...
                let ok = mt32RomDirPath.withCString { pathPtr in
                    imwrap_engine_enable_mt32(engineHandle, pathPtr, &errorBuffer, errorBuffer.count)
                }
                if ok == 1 {
                    errorMessage = ""
                    startAudioOutput()
...
public struct IMWrapPlayerView: View {
        ...
        NavigationSplitView {
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                HStack(alignment: .top, spacing: 16) {
                    VStack(alignment: .leading, spacing: 12) {
                        GroupBox("Profil de rendu MIDI") {
                            Picker("Profil", selection: $model.selectedProfile) {
                                Text("General MIDI").tag(IMWrapAuthoringProfileGeneralMidi)
                                Text("Roland MT-32").tag(IMWrapAuthoringProfileMt32)
                            }
                            .pickerStyle(.segmented)
                            .padding(.vertical, 4)
                        }
                    ...
                    GroupBox("Déclencheurs iMWrap (Hooks)") {
                        VStack(spacing: 8) {
                            HStack {
                                VStack(alignment: .leading, spacing: 2) {
                                    Text("Classe")
                ...
                // Events List
                GroupBox("Événements iMWrap & SysEx du son sélectionné") {
                    if model.events.isEmpty {
                        Text("Aucun événement iMWrap détecté pour ce profil.")
                            .font(.subheadline)
                            .foregroundStyle(.secondary)
