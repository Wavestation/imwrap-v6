...
{
    internal static class IMWrapBankCodec
    {
        ...
        private sealed class ParsedSmfSequence
        {
            ...
            public List<IMWrapTrackModel> Tracks;
        ...
        private sealed class MergedEvent
        {
            ...
            public int Order;
            public IMWrapMidiEventModel Event;
        }

        public static IMWrapBankProjectModel LoadFromFile(string fullPath, string relativePath)
        {
            byte[] bytes = File.ReadAllBytes(fullPath);
            return LoadFromBytes(bytes, relativePath);
        }

        public static IMWrapBankProjectModel LoadFromBytes(byte[] bytes, string relativePath)
        {
            if (bytes == null)
            {
            ...
            IMWrapBankProjectModel bank = new IMWrapBankProjectModel();
        ...
        public static byte[] SaveToBytes(IMWrapBankProjectModel bank)
        {
            if (bank == null)
            {
            ...
            List<IMWrapSoundModel> sounds = new List<IMWrapSoundModel>(bank.Sounds);
            sounds.Sort(delegate(IMWrapSoundModel left, IMWrapSoundModel right)
            {
                return left.SoundId.CompareTo(right.SoundId);
            });
        ...
        public static List<IMWrapTrackModel> ImportMidiTracks(string[] filePaths, out ushort division)
        {
            if (filePaths == null || filePaths.Length == 0)
            {
                division = 480;
                return new List<IMWrapTrackModel>();
            }

            List<IMWrapTrackModel> allTracks = new List<IMWrapTrackModel>();
            ...
            for (int pathIdx = 0; pathIdx < filePaths.Length; pathIdx++)
            {
                ...
                {
                    IMWrapTrackModel mergedTrack = MergeTracksToFormat0(sequence.Tracks);
                ...
                else
                {
                    ...
                    {
                        IMWrapTrackModel track = CloneTrack(sequence.Tracks[i]);
        ...
        private static IMWrapSoundModel ParseSound(byte[] bytes, DirectoryEntry directoryEntry)
        {
        ...
        private static IMWrapVariantModel ParseVariant(byte[] bytes, string chunkId, int bodyOffset, int bodyLength)
        {
            int offset = bodyOffset;
            int endOffset = bodyOffset + bodyLength;

            IMWrapVariantModel variant = new IMWrapVariantModel();
        ...
        private static ParsedSmfSequence ParseSmf(byte[] bytes)
        {
            ...
            for (int trackIndex = 0; trackIndex < trackCount; ++trackIndex)
            {
                ...
                IMWrapTrackModel track = ParseTrack(bytes, offset, (int)trackLength, trackIndex);
        ...
        private static IMWrapTrackModel ParseTrack(byte[] bytes, int offset, int length, int trackIndex)
        {
            IMWrapTrackModel track = new IMWrapTrackModel();
            ...
            while (offset < endOffset)
            {
                ...
                IMWrapMidiEventModel midiEvent = new IMWrapMidiEventModel();
                midiEvent.Tick = tick;
                ...
                if (status == 0xFF)
                {
                    midiEvent.Type = IMWrapMidiEventType.Meta;
                ...
                {
                    midiEvent.Type = IMWrapMidiEventType.SysEx;
                    midiEvent.Status = status;
                    uint payloadLength = ReadVlq(bytes, ref offset, endOffset);
                    if ((offset + payloadLength) > endOffset)
                    {
                ...
                {
                    midiEvent.Type = IMWrapMidiEventType.Channel;
                    midiEvent.Status = status;
                ...
                midiEvent.Type = IMWrapMidiEventType.System;
                midiEvent.Status = status;
                runningStatus = 0;
        ...
        private static IMWrapTrackModel MergeTracksToFormat0(List<IMWrapTrackModel> tracks)
        {
            ...
            {
                List<IMWrapMidiEventModel> events = tracks[trackIndex].Events;
                uint absoluteTick = 0;
                for (int eventIndex = 0; eventIndex < events.Count; ++eventIndex)
                {
                    absoluteTick += events[eventIndex].Delta;
                    if (events[eventIndex].Type == IMWrapMidiEventType.Meta && events[eventIndex].MetaType == 0x2F)
                    {
                        continue;
                    }
            ...
            IMWrapTrackModel mergedTrack = new IMWrapTrackModel();
            ...
            {
                IMWrapMidiEventModel midiEvent = mergedEvents[i].Event;
            ...
            IMWrapMidiEventModel endOfTrack = new IMWrapMidiEventModel();
            endOfTrack.Type = IMWrapMidiEventType.Meta;
        ...
        private static byte[] BuildSoundChunk(IMWrapSoundModel sound, out ushort variantMask)
        {
            List<byte> body = new List<byte>();
            ...
            {
                IMWrapVariantModel variant = sound.Variants[i];
        ...
        private static byte[] BuildVariantChunk(IMWrapVariantModel variant)
        {
            List<byte> body = new List<byte>();
        ...
        private static byte[] SerializeVariantSequence(IMWrapVariantModel variant)
        {
            ...
            for (int i = 0; i < variant.Tracks.Count; ++i)
            {
                ...
                {
                    IMWrapMidiEventModel midiEvent = track.Events[eventIndex];
                    WriteVlq(trackBytes, midiEvent.Delta);
                    trackBytes.Add(midiEvent.Status);

                    if (midiEvent.Type == IMWrapMidiEventType.Channel)
                    {
        ...
        private static IMWrapVariantKind VariantKindFromChunkId(string chunkId)
        {
            if (chunkId == "GMD ")
            {
        ...
        private static string ChunkIdFromVariantKind(IMWrapVariantKind kind)
        {
            if (kind == IMWrapVariantKind.GeneralMidi)
            {
                return "GMD ";
            }

            if (kind == IMWrapVariantKind.RolandMt32)
            {
                return "ROL ";
            }

            if (kind == IMWrapVariantKind.Adlib)
            {
                return "ADL ";
            }
        ...
        private static IMWrapTrackModel CloneTrack(IMWrapTrackModel source)
        {
            IMWrapTrackModel track = new IMWrapTrackModel();
        ...
        private static IMWrapMidiEventModel CloneEvent(IMWrapMidiEventModel source)
        {
            IMWrapMidiEventModel midiEvent = new IMWrapMidiEventModel();
