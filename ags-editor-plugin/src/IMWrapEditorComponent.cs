...
{
    internal sealed class IMWrapEditorComponent : IEditorComponent
    {
        private sealed class OpenBankDocument
        {
            public string RelativePath;
            public ContentDocument Document;
            public IMWrapEditorPane Pane;
        }

        private const string RootNodeId = "IMWrap.Root";
        private const string BanksNodeId = "IMWrap.Banks";
        private const string RootMenuId = "IMWrapMenu";
        private const string RootIconKey = "IMWrap.RootIcon";
        private const string FolderIconKey = "IMWrap.FolderIcon";
        private const string FileIconKey = "IMWrap.FileIcon";
        private const string EditorIconKey = "IMWrap.EditorIcon";
        private const string FileNodePrefix = "IMWrap.File:";
        ...
        private const string XmlNodeName = "IMWrapEditor";
        ...
        public IMWrapEditorComponent(IAGSEditor editor)
        {
            _editor = editor;
            ...
            _gui.RegisterIcon(RootIconKey, EmbeddedIconLoader.LoadPngIcon("AgsIMWrap.Editor.Icons.imwrap.png", SystemIcons.Application));
            _gui.RegisterIcon(FolderIconKey, EmbeddedIconLoader.LoadPngIcon("AgsIMWrap.Editor.Icons.bank_folder.png", SystemIcons.Shield));
            _gui.RegisterIcon(FileIconKey, EmbeddedIconLoader.LoadPngIcon("AgsIMWrap.Editor.Icons.bank_element.png", SystemIcons.Information));
            _gui.RegisterIcon(EditorIconKey, EmbeddedIconLoader.LoadPngIcon("AgsIMWrap.Editor.Icons.packer_tab.png", SystemIcons.Application));

            _gui.AddMenu(this, RootMenuId, "&iMWrap", _gui.FileMenuID);
            _mainMenuCommands = new MenuCommands(RootMenuId);
            _mainMenuCommands.Commands.Add(new MenuCommand(CommandOpenEditor, "&Open iMWrap Packer", EditorIconKey));
            ...
            _gui.ProjectTree.AddTreeRoot(this, RootNodeId, "iMWrap", RootIconKey);
            RefreshTree();
        }

        public string ComponentID
        {
            get { return "IMWrapEditorComponent"; }
        }
        ...
        public IList<MenuCommand> GetContextMenu(string controlID)
        {
            ...
            if (_fileNodePaths.TryGetValue(controlID, out relativePath))
            {
                ...
                commands.Add(new MenuCommand(CommandOpenSelected, "Open in iMWrap Packer", EditorIconKey));
            ...
            commands.Add(new MenuCommand(CommandOpenEditor, "Open iMWrap Packer", EditorIconKey));
            commands.Add(new MenuCommand(CommandCreateBank, "New Bank...", FolderIconKey));
            commands.Add(new MenuCommand(CommandRefreshBanks, "Refresh Banks", FileIconKey));
            return commands;
        }
        ...
        private void OpenBank(string relativePath)
        {
            ...
            IMWrapBankProjectModel model;
            try
            {
                model = IMWrapBankCodec.LoadFromFile(fullPath, relativePath);
            }
            catch (Exception ex)
            {
            ...
            documentContext.Pane = new IMWrapEditorPane();
            // Disabled for now: persistence proved unreliable inside AGS.
        ...
        private void RefreshDocument(OpenBankDocument documentContext)
        {
            ...
            try
            {
                IMWrapBankProjectModel model = IMWrapBankCodec.LoadFromFile(BuildFullPath(documentContext.RelativePath), documentContext.RelativePath);
                documentContext.Pane.LoadBank(model);
        ...
        private void CreateBankFromCurrentContext()
        {
            ...
            try
            {
                IMWrapBankFileService.CreateEmptyBank(fullPath);
                RefreshTree();
                OpenBank(relativePath);
            }
            catch (Exception ex)
